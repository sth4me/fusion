...
func TestE2E_EngineBasic(t *testing.T) {
	...
	u.Name.Set("alice")
	u.Age.Set(30)
	if err := engine.Insert(Users, u).Exec(context.Background()); err != nil {
	...
	// 更新（无 Where → 自动按主键）
	got.Age.Set(31)
	if err := engine.Update(Users, &got).Exec(context.Background()); err != nil {
		t.Fatalf("update: %v", err)
	}
	again, _ := engine.From(Users).Where(Users.Proto.ID.Eq(1)).One(context.Background())
	if again.Age.Get() != 31 {
...
func TestE2E_EngineMultiIsolation(t *testing.T) {
	...
	u1.Name.Set("from-e1")
	u1.Age.Set(1)
	e1.Insert(Users, u1).Exec(context.Background())
	u2 := &EUser{}
	u2.Name.Set("from-e2")
	u2.Age.Set(2)
	e2.Insert(Users, u2).Exec(context.Background())

	// e1 只能看到 from-e1，e2 只能看到 from-e2
	got1, _ := e1.From(Users).OrderBy(Users.Proto.ID.Asc()).All(context.Background())
	if len(got1) != 1 || got1[0].Name.Get() != "from-e1" {
		t.Errorf("e1 should see only from-e1, got %+v", got1)
	}
	got2, _ := e2.From(Users).OrderBy(Users.Proto.ID.Asc()).All(context.Background())
...
func TestE2E_EnginePerEngineLogger(t *testing.T) {
	...
	u := &EUser{}
	u.Name.Set("a")
	e1.Insert(Users, u).Exec(e1.Ctx(context.Background()))
	e2.From(Users).All(e2.Ctx(context.Background()))
...
func TestE2E_EngineTx(t *testing.T) {
	...
	err := engine.Tx(context.Background(), func(ctx context.Context) error {
		u := &EUser{}
		u.Name.Set("tx-commit")
		u.Age.Set(1)
		return engine.Insert(Users, u).Exec(ctx)
	})
	if err != nil {
		t.Fatalf("ETx commit: %v", err)
	}
	all, _ := engine.From(Users).All(context.Background())
	...
	// 回滚后仍只有 1 条
	all2, _ := engine.From(Users).All(context.Background())
	if len(all2) != 1 {
