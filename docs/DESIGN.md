...
**结论**：`Where` 只接受 `Expr`；多次调用按 **AND 叠加**（等价于 `Where(c1.And(c2))`）。布尔逻辑（OR/优先级）由表达式构建器负责，AND/OR 优先级由表达式树结构显式表达。

**推理**：`Where(c1).Where(c2)` 的调用形式与主流 ORM（GORM/ent）一致，符合"条件式叠加过滤"的心智模型。若多次调用是"覆盖"语义，链式叠加会静默丢条件（不报错、无警告，结果集悄悄变宽）——2026-08 挂单列表因此返回了非 held 状态的行。OR 等复杂逻辑仍需显式 `Expr.Or` 表达，防止隐式 AND 与 OR 混用产生歧义。
